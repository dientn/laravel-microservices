...
class CombineDocs extends Command
{
    ...
    private function getResources()
    {
        return collect($this->config['services'])->map(function($settings, $serviceId) {
            ...
            if (! (isset($data['apis']) || isset($data['paths']))) throw new DataFormatException($serviceId . ' doesn\'t contain API data');

            $apis = isset($data['apis'])? $data['apis'] : $this->parseSwaggerTwo($data);
            return collect($apis)->map(function ($api) use ($url, $serviceId, $docRoot) {
    ...
    private function getPaths(Collection $resources)
    {
        return $resources->reduce(function($carry, $resource) {
            ...
            $apis = isset($data['apis'])? $data['apis'] : $this->parseSwaggerTwo($data);
            $apis = collect($apis)->map(function ($api) use ($resource) {
