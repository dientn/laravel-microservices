...
class ParseServices extends Command
{
    ...
    private function getRoot()
    {
        return collect($this->config['services'])->map(function($settings, $serviceId) {
            ...
            $docRoot = $settings['doc_point'] ?? $this->config[self::GLOBAL]['doc_point'];
            $hostname = $settings['hostname'] ?? $serviceId . '.' . $this->config[self::GLOBAL]['domain'];
            $url = $settings['url'] ?? 'http://' . $hostname;

            $response = $this->client->request('GET', $url . $docRoot, ['timeout' => 10.0]);

            $dataRoot = json_decode((string) $response->getBody(), true);

            if ($dataRoot === null) {
                throw new DataFormatException('Unable to get JSON response from ' . $serviceId);
            }

            if (isset($dataRoot['swaggerVersion']) && isset($dataRoot['apis']) && preg_match('/^1\./', $dataRoot['swaggerVersion'])) {
                return $this->injectSettings($this->parseSwaggerResources($dataRoot['apis']), $url, $docRoot, $serviceId);
            }

            if (isset($dataRoot['swagger']) && preg_match('/^2\./', $dataRoot['swagger']) && isset($dataRoot['paths'])) {
                return $this->injectSettings($this->parseSwaggerTwo($dataRoot), $url, $docRoot, $serviceId);
            }
    ...
    protected function parseSwaggerTwo(array $swagger)
    {
        ...
        return collect($swagger['paths'])->map(function($data, $path) use ($basePath) {
            return [
                ...
                'privileges'=> isset($path['x-authorizes'])? $path['x-authorizes'] : 'all',
    ...
    private function getPaths(Collection $resources)
    {
        return $resources->reduce(function($carry, $resource) {
            ...
            if (! isset($resource['swagger2-data'])) {
                ...
                if ($data === null) {
                    throw new DataFormatException('Unable to get JSON response from ' . $resource['serviceId']);
                }
    ...
    public function handle()
    {
        ...
        if (empty($output)) {
            throw new DataFormatException('Unable to parse the routes');
        }
